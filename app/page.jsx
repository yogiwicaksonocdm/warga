...
const initialMockData = [
  ...
  {
    id: '2',
    nik: '3201015608980003',
    noKK: '3201011210150002',
    nama: 'Siti Aminah',
    gender: 'Perempuan',
    tempatLahir: 'Bogor',
    tanggalLahir: '1998-08-16',
    rt: '001',
    rw: '005',
...
export default function DashboardWarga() {
  ...
  const filteredWarga = wargaList.filter((item) => {
    ...
    const matchesRT = filterRT === 'ALL' || item.rt === filterRT;
    return matchesSearch && matchesRT;
  });

  const totalWarga = wargaList.length;
  const totalLaki = wargaList.filter((w) => w.gender === 'Laki-Laki').length;
            ...
            >
              <span
                className={`w-2 h-2 mr-1.5 rounded-full ${
                  isFirebaseConfigured ? 'bg-white animate-pulse' : 'bg-slate-800'
                }`}
              ></span>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex border-b border-slate-300 mb-6 bg-white rounded-t-xl px-4 pt-2">
          <button
            ...
            className={`py-3 px-5 font-semibold text-sm transition-all border-b-2 ${
              activeTab === 'list'
